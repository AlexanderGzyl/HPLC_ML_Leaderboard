...
HPLC_all=pd.read_csv('sourcedata/All_column_charity.csv')
bad_all_index=np.load('sourcedata/bad_all_column.npy') #Some compounds that cannot get 3D conformer by RDKit
...
dataset_all=np.load('sourcedata/dataset_charity_all_column.npy',allow_pickle=True).tolist()
...
# save indexes into a file in order to see what was used
    ...
    np.savez(filename, train=train_index, valid=valid_index, test =test_index )
    print('data saved')
